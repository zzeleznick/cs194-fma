...
Now that we have seen Gaussian and Laplacian Stacks in action, we can now use masks and blend two composite images together. The first mask I implemented was a trivial blend using a vertical seam and a sum of each levels along the laplacian pyramids between each image. Next, I created my own mask, which I then mapped to a Gaussian stack and repeated the process.
